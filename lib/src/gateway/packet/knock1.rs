...
impl<B: AsRef<[u8]>> fmt::Debug for Knock1<B> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("rimnet::packet::Knock1")
...
impl<B: AsRef<[u8]>> AsRef<[u8]> for Knock1<B> {
    fn as_ref(&self) -> &[u8] {
        &self.buffer.as_ref()
    }
}

impl<B: AsRef<[u8]>> Knock1<B> {
    pub fn unchecked(buffer: B) -> Knock1<B> {
        Knock1 { buffer }
    }
...
#[derive(Debug)]
pub struct Knock1PacketBuilder {
    private_ipv4: Option<Ipv4Addr>,
    public_ipv4: Option<Ipv4Addr>,
    public_port: u16,
}

impl Knock1PacketBuilder {
    pub fn new() -> Result<Self> {
        Ok(Knock1PacketBuilder {
            private_ipv4: None,
            public_ipv4: None,
            public_port: 0,
        })
    ...
    pub fn build(self) -> Result<Knock1<Vec<u8>>> {
...
impl Default for Knock1PacketBuilder {
    fn default() -> Self {
        Knock1PacketBuilder::new().unwrap()
    }
}
